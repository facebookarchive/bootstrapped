...
def bootstrap(values, stat_func, denominator_values=None, alpha=0.05,
    ...
    Args:
        ...
        stat_func: statistic to bootstrap. We provide two default functions:
                * bootstrapped.mean
                * bootstrapped.sum
            Note, custom aggregate functions may be passed. The function must
                take a 2d numpy.array and aggregate across rows.  The function
                should return an array of doubles. If you are using numpy
                aggregation functions (np.mean, np.std, np.percentile) then
                set the axis argument equal to 1 (aggregate columns).

                def aggregate_mean(values):
                    return np.std(values, axis=1)
...
def bootstrap_ab(test, ctrl, stat_func, compare_func, test_denominator=None,
    ...
    Args:
        ...
        stat_func: statistic to bootstrap. Defaults to mean.
            Note, custom aggregate functions may be passed. The function must
                take a 2d numpy.array and aggregate across rows.  The function
                should return an array of doubles. If you are using numpy
                aggregation functions (np.mean, np.std, np.percentile) then
        ...
        compare_func: Function to compare test and control against. Defaults to
            percent_change.
